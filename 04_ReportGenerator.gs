...
function fetchAiReview(reportData) {
  ...
  // ★修正：ご要望の分析ルールを完全にプロンプトへ組み込みました
  const prompt = `あなたは医療法人のデータアナリストです。以下の月次実績データを分析し、Chatworkで報告するためのAIレビューを生成してください。
...
・以下の構成で3点を記述すること：
 ...
 2点目：「稼働人員」「構成比（特に紹介会社や直応募）」「新規採用」など、医師周りの実績にフォーカスした振り返り。
 3点目：次月への「提言（解決策）」は書かず、データから読み取れる「次月の注目項目（引き続き注視していくべきポイント）」を述べること。
・数値を引用する際は、「前月比」と「昨年比」を比較し、変化幅（インパクト）がより大きい方を積極的に採用して言及すること。
・ネガティブな表現（例：「圧迫している」「悪化した」等）は避け、事実を客観的かつ前向きなトーンで記述すること。
・全体の文字数は200〜300文字程度で端的にまとめること。
